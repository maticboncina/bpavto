...
class CarPostingScheduler {
    constructor() {
        ...
        this.MIN_DELAY = 45000; // 45 seconds minimum between posts
        this.MAX_DELAY = 180000; // 3 minutes maximum between posts
        this.RETRY_DELAY = 300000; // 5 minutes for retries
        this.MAX_ATTEMPTS = 3;
     ...
     */
    addScheduledJob(type, data, scheduledFor, callback) {
    ...
    preCreateRepeatJobs(baseJob, repeatConfig, count = 5) {
     ...
     */
    notifyStatus(jobId, status, message) {
     ...
     */
    getQueueStatus() {
        const nextJob = this.queue.reduce((earliest, job) => !earliest || job.scheduledFor < earliest.scheduledFor ? job : earliest, null);
        return {
...
window.getSchedulerStatus = () => scheduler.getQueueStatus();
const make_car = async (car, images) => {
    ...
    try {
        ...
        const keysToRemove = Object.keys(allKeys).filter(key => key.startsWith('pendingCarData') ||
    ...
    return new Promise((resolve) => {
        ...
        window.pendingJobResolve = resolve;
                ...
                if (makeDropdown && car.make) {
                    ...
                    if (modelDropdown && car.model && !modelDropdown.disabled) {
                        ...
                        if (found) {
                            ...
                            await humanDelay(4000, 8000);
                            // Fill month dropdown
                            const monthDropdown = document.querySelector('select[name="mesec"]');
                            ...
                            // Fill year dropdown
                            const yearDropdown = document.querySelector('select[name="leto"]');
                            ...
                            if (car.fuel) {
                                ...
                                const fuelRadio = document.querySelector(`input[name="gorivo"][value="${car.fuel}"]`);
                                if (fuelRadio) {
                            ...
                            const continueButton = document.querySelector('button[name="potrdi"][value="1"]');
                            if (continueButton) {
    ...
    setTimeout(async () => {
        try {
            ...
            if (result.shouldClickManualEntry) {
                const manualEntryLink = document.querySelector('a.supurl[href*="ad_insert_car_step3.asp"]');
                if (manualEntryLink) {
...
chrome.runtime.onMessage.addListener(async (msg) => {
    ...
    try {
        ...
        for (let i = startIndex; i < totalImages; i++) {
            ...
            const fileInput = document.querySelector('#gallery-photo-add');
            if (!fileInput) {
        ...
        const finishButton = document.querySelector('button[name="ADVIEW"][value="1"]');
        if (finishButton) {
        ...
        const jobResolve = window.pendingJobResolve;
        if (jobResolve) {
            jobResolve('ui_automation_' + Date.now());
            delete window.pendingJobResolve;
...
async function uploadFileToInput(input, file, photoNumber) {
    try {
        ...
        const addButton = document.querySelector(`input[type="submit"][name="gumb${photoNumber}"]`);
        if (addButton) {
            ...
            const retryButton = document.querySelector(`input[type="submit"][name="gumb${photoNumber}"]`);
            if (retryButton) {
    ...
    setTimeout(async () => {
        try {
            ...
            if (result.pendingCarData) {
                ...
                const typeField = document.querySelector('input[name="tipvozila"]');
                ...
                const vinField = document.querySelector('input[name="VIN"]');
                if (vinField) {
                ...
                const publishVinCheckbox = document.querySelector('input[name="VINobjavi"][type="checkbox"]');
                ...
                // Guarantee checkbox
                const guaranteeCheckbox = document.querySelector('input[name="guarantee"][type="checkbox"]');
                ...
                // Warranty checkbox
                const warrantyCheckbox = document.querySelector('input[name="jamstvo"][type="checkbox"]');
                if (warrantyCheckbox) {
                    // Checkbox: 1400-2400ms
                    await humanDelay(1400, 2400);
                ...
                // Oldtimer checkbox
                const oldtimerCheckbox = document.querySelector('input[name="oldtimer"][type="checkbox"]');
                if (oldtimerCheckbox) {
                    // Checkbox: 1400-2400ms
                    await humanDelay(1400, 2400);
                ...
                await humanDelay(4000, 8000);
                // Select age dropdown
                const ageDropdown = document.querySelector('select[name="starost"]');
                ...
                // Fill mileage field
                const mileageField = document.querySelector('input[name="prevozenikm"]');
                ...
                const techMonthDropdown = document.querySelector('select[name="tehmesec"]');
                ...
                const techYearField = document.querySelector('input[name="tehleto"]');
                ...
                const ownersDropdown = document.querySelector('select[name="lastnikov"]');
                ...
                if (car.offer_type !== undefined) {
                    // Radio button: 1400-2400ms
                    await humanDelay(1400, 2400);
                    const offerTypeRadio = document.querySelector(`input[name="najem"][value="${car.offer_type}"]`);
                    if (offerTypeRadio) {
                ...
                // Fill price field
                const priceField = document.querySelector('input[name="cena"]');
                ...
                const gotpopustCheckbox = document.querySelector('input[name="gotpopust"][type="checkbox"]');
                ...
                const zadcenaCheckbox = document.querySelector('input[name="zadcena"][type="checkbox"]');
                if (zadcenaCheckbox) {
                    zadcenaCheckbox.checked = car.final_price || false;
                    console.log(`[UI Automation] Set final price to: ${car.final_price}`);
                }
                const kreditCheckbox = document.querySelector('input[name="kredit"][type="checkbox"]');
                if (kreditCheckbox) {
                    kreditCheckbox.checked = car.credit || false;
                    console.log(`[UI Automation] Set credit to: ${car.credit}`);
                }
                const leasingCheckbox = document.querySelector('input[name="leasing"][type="checkbox"]');
                if (leasingCheckbox) {
                    leasingCheckbox.checked = car.leasing || false;
                    console.log(`[UI Automation] Set leasing to: ${car.leasing}`);
                }
                const prevzemleasCheckbox = document.querySelector('input[name="prevzemleas"][type="checkbox"]');
                ...
                const naobrokeCheckbox = document.querySelector('input[name="naobroke"][type="checkbox"]');
                if (naobrokeCheckbox) {
                    naobrokeCheckbox.checked = car.installments || false;
                    console.log(`[UI Automation] Set installments to: ${car.installments}`);
                }
                const menjavaCheckbox = document.querySelector('input[name="menjava"][type="checkbox"]');
                if (menjavaCheckbox) {
                    menjavaCheckbox.checked = car.exchange || false;
                    console.log(`[UI Automation] Set exchange to: ${car.exchange}`);
                }
                const eutaxCheckbox = document.querySelector('input[name="eutax"][type="checkbox"]');
                if (eutaxCheckbox) {
                    eutaxCheckbox.checked = car.eutax || false;
                    console.log(`[UI Automation] Set EU tax to: ${car.eutax}`);
                }
                const prodanoCheckbox = document.querySelector('input[name="prodano"][type="checkbox"]');
                if (prodanoCheckbox) {
                ...
                const internalIdField = document.querySelector('input[name="interniID"]');
                ...
                const publishInternalIdCheckbox = document.querySelector('input[name="objaviID"][type="checkbox"]');
                ...
                const administratorField = document.querySelector('input[name="administratoroglasa"]');
                ...
                const soldWholeRadio = document.querySelector(`input[name="prodamvceloti"][value="${car.sold_whole ? '1' : '0'}"]`);
                if (soldWholeRadio) {
                    soldWholeRadio.checked = true;
                    console.log(`[UI Automation] Set sold as whole to: ${car.sold_whole ? '1' : '0'}`);
                }
                // Set driveable radio button
                const driveableRadio = document.querySelector(`input[name="vozno"][value="${car.driveable ? '1' : '0'}"]`);
                if (driveableRadio) {
                    driveableRadio.checked = true;
                    console.log(`[UI Automation] Set driveable to: ${car.driveable ? '1' : '0'}`);
                }
                // Set damaged radio button
                const damagedRadio = document.querySelector(`input[name="poskodovan"][value="${car.damaged ? '1' : '0'}"]`);
                if (damagedRadio) {
                    damagedRadio.checked = true;
                    console.log(`[UI Automation] Set damaged to: ${car.damaged ? '1' : '0'}`);
                }
                // Set carambolised radio button
                const carambolisedRadio = document.querySelector(`input[name="karambolirano"][value="${car.carambolised ? '1' : '0'}"]`);
                ...
                const inFailureCheckbox = document.querySelector('input[name="okvara"][type="checkbox"]');
                ...
                const floodedCheckbox = document.querySelector('input[name="poplavljeno"][type="checkbox"]');
                if (floodedCheckbox) {
                ...
                const serviceBookCheckbox = document.querySelector('input[name="SK"][type="checkbox"]');
                ...
                const serviceBookConfirmedCheckbox = document.querySelector('input[name="SKpotrjena"][type="checkbox"]');
                ...
                const slovenianOriginCheckbox = document.querySelector('input[name="SLOporeklo"][type="checkbox"]');
                ...
                const garagedCheckbox = document.querySelector('input[name="gar"][type="checkbox"]');
                if (garagedCheckbox) {
                ...
                const notInCollisionCheckbox = document.querySelector('input[name="nek"][type="checkbox"]');
                ...
                const powertrainField = document.querySelector('input[name="PowertrainTXT"]');
                ...
                const gearboxDropdown = document.querySelector('select[name="menjalnik"]');
                ...
                const enginePowerField = document.querySelector('input[name="kw"]');
                ...
                const engineDisplacementField = document.querySelector('input[name="ccm"]');
                ...
                await new Promise(resolve => setTimeout(resolve, 500));
                // Fill color dropdown
                const colorDropdown = document.querySelector('select[name="barva"]');
                ...
                const metallicCheckbox = document.querySelector('input[name="kovinska"][type="checkbox"]');
                if (metallicCheckbox) {
                ...
                const matteCheckbox = document.querySelector('input[name="BarvaMatt"][type="checkbox"]');
                if (matteCheckbox) {
                ...
                await new Promise(resolve => setTimeout(resolve, 500));
                // Fill roof dropdown
                const roofDropdown = document.querySelector('select[name="streha"]');
                ...
                const electricRoofCheckbox = document.querySelector('input[name="elstreha"][type="checkbox"]');
                ...
                // Fill doors dropdown
                const doorsDropdown = document.querySelector('select[name="vrata"]');
                ...
                const slidingDoorCheckbox = document.querySelector('input[name="dostvrat04"][type="checkbox"]');
                ...
                const seatsField = document.querySelector('input[name="sedezev"]');
                ...
                const upholsteryDropdown = document.querySelector('select[name="oblazinjenje"]');
                ...
                const upholsteryColorDropdown = document.querySelector('select[name="barvaoblazin"]');
                ...
                const fuelConsumptionField = document.querySelector('input[name="porabaKombi"]');
                ...
                const emissionsStandardField = document.querySelector('input[name="EmisijeStandard"]');
                ...
                const co2EmissionsField = document.querySelector('input[name="CO2"]');
                ...
                const publishConsumptionCheckbox = document.querySelector('input[name="porabaOBJAVI"][type="checkbox"]');
                ...
                if (car.chassis_features) {
                    const chassisFeatures = car.chassis_features.split(',').map((feature) => feature.trim());
                    console.log(`[UI Automation] Processing chassis features: ${chassisFeatures.join(', ')}`);
                    chassisFeatures.forEach((feature) => {
                        const checkbox = document.querySelector(`input[name="${feature}"][type="checkbox"]`);
                        if (checkbox) {
                ...
                // Toggle ALU checkbox
                const aluCheckbox = document.querySelector('input[name="ALU"][type="checkbox"]');
                if (aluCheckbox) {
                ...
                await new Promise(resolve => setTimeout(resolve, 500));
                // Fill airbags field
                const airbagsField = document.querySelector('input[name="airbagov"]');
                ...
                if (car.varnosti) {
                    const securityFeatures = car.varnosti.split(',').map((feature) => feature.trim());
                ...
                if (car.inside) {
                    const interiorFeatures = car.inside.split(',').map((feature) => feature.trim());
                ...
                if (car.comfort) {
                    const comfortFeatures = car.comfort.split(',').map((feature) => feature.trim());
                ...
                const carRadioCheckbox = document.querySelector('input[name="avtoradio"][type="checkbox"]');
                if (carRadioCheckbox) {
                    carRadioCheckbox.checked = car.car_radio || false;
                    console.log(`[UI Automation] Set car radio to: ${car.car_radio}`);
                }
                // Toggle CD checkbox
                const cdCheckbox = document.querySelector('input[name="cd"][type="checkbox"]');
                if (cdCheckbox) {
                    cdCheckbox.checked = car.cd_disk || false;
                    console.log(`[UI Automation] Set CD to: ${car.cd_disk}`);
                }
                // Toggle HiFi checkbox
                const hifiCheckbox = document.querySelector('input[name="hifi"][type="checkbox"]');
                if (hifiCheckbox) {
                ...
                if (car.multimedia) {
                    const multimediaFeatures = car.multimedia.split(',').map((feature) => feature.trim());
                    console.log(`[UI Automation] Processing multimedia features: ${multimediaFeatures.join(', ')}`);
                    multimediaFeatures.forEach((feature) => {
                        const checkbox = document.querySelector(`input[name="${feature}"][type="checkbox"]`);
                        if (checkbox) {
                            checkbox.checked = true;
                ...
                if (car.appliance) {
                    const applianceFeatures = car.appliance.split(',').map((feature) => feature.trim());
                    console.log(`[UI Automation] Processing appliance features: ${applianceFeatures.join(', ')}`);
                    applianceFeatures.forEach((feature) => {
                        const checkbox = document.querySelector(`input[name="${feature}"][type="checkbox"]`);
                        if (checkbox) {
                            checkbox.checked = true;
                ...
                const commentsTextarea = document.querySelector('textarea[name="opombe"]');
                ...
                if (car.delivery_time !== undefined) {
                    const deliveryRadio = document.querySelector(`input[name="rokdobave"][value="${car.delivery_time}"]`);
                    if (deliveryRadio) {
                ...
                const addPhotosButton = document.querySelector('button[name="EDITAD"][value="1"]');
                if (addPhotosButton) {
                    // Final delay before clicking
                    await humanDelay(2000, 4000);
...
if (window.location.href.includes('ad_confirm.asp')) {
    ...
    setTimeout(async () => {
        ...
        const regularAdLink = document.querySelector('a[href*="ad_reactivate.asp"]');
        if (regularAdLink) {
...
chrome.runtime.onMessage.addListener((msg, _, sendResponse) => {
    ...
    if (msg.type === "create") {
        ...
        if (scheduledFor) {
            jobId = scheduler.addScheduledJob('create', jobData, scheduledFor, (status, message) => {
                console.log(`[Create Job] Status: ${status}${message ? ` - ${message}` : ''}`);
            });
        }
        else {
            jobId = scheduler.addJob('create', jobData, (status, message) => {
                console.log(`[Create Job] Status: ${status}${message ? ` - ${message}` : ''}`);
            });
        }
    ...
    if (msg.type === "republish") {
        ...
        let jobId;
        if (msg.scheduledFor) {
            jobId = scheduler.addScheduledJob('republish', jobData, msg.scheduledFor, (status, message) => {
                console.log(`[Republish Job] Status: ${status}${message ? ` - ${message}` : ''}`);
            });
        }
        else {
            jobId = scheduler.addJob('republish', jobData, (status, message) => {
                console.log(`[Republish Job] Status: ${status}${message ? ` - ${message}` : ''}`);
            });
        }
    ...
    if (msg.type === "is_logged_in") {
        ...
        const postAdButton = document.querySelector("a[href*='ad_insert']") ||
            ...
            Array.from(document.querySelectorAll('button')).find(btn => btn.textContent?.toLowerCase().includes('post'));
...
const deletePost = async (id, redirect = true) => {
    const resp = await fetch(`https://www.avto.net/_2016mojavtonet/ad_delete.asp?id=${encodeURIComponent(id)}`, {
        method: "GET",
    });
    console.log("delete post, status code:", resp.status);
    if (resp.status === 200) {
        if (redirect)
            document.location.replace("https://www.avto.net/_2016mojavtonet/results321_makes.asp?arhiv=1");
        return;
    }
    alert("Delete failed");
};
